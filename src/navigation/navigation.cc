...
void Navigation::UpdateOdometry(const Vector2f& loc,
    ...
    robot_dist_traveled_ += (vel.x() * (time_step));//sqrt(pow(loc.x() - robot_loc_.x(), 2) + pow(loc.y() - robot_loc_.y(), 2));
    robot_vel_ = vel;
    robot_angle_ = angle;
    robot_loc_ = loc;
...
void Navigation::Run(float delta_x) {
    cout << "vel: " << robot_vel_.x() << " dist: " << robot_dist_traveled_ << " prev state: " << robot_prev_state << endl;
    float vel = 0; //robot_vel_.x();
    ...
    if (robot_dist_traveled_ < dist_thresh_x1) {
        ...
        vel = robot_vel_.x() + (acc * time_step);
        ...
        if (robot_dist_traveled_ < delta_x){
            vel = robot_vel_.x() - (max_decc * time_step);
            //robot_dist_traveled_ += robot_vel_.x() * time_step;
        } else {
           vel = 0;
        }
